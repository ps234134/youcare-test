        ...
        <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-200">
            <div class="container mx-auto px-6 py-1">
                <div class="bg-white shadow-md rounded my-6 p-5">
                    ...
                    <form method="POST" action="{{ route('rooster.store') }}">
                        @csrf
                        @method('post')
                        <div class="flex flex-col space-y-2">
                            <label for="shift" class="text-gray-700 select-none font-medium">Dienst
                                datum</label>
                            ...
                            <div>
                                ...
                                <select
                                    class="px-4 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-200 w-auto"
                                    id="user" name="user" required>
                                    @foreach ($users as $user)
                                    <option value="{{ $user->id }}">{{ $user->name }} - {{ $user->roles->first()->name }}
                                    @endforeach
                                </select>
                            </div>
                        </div>
